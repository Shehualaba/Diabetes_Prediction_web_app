...
HighBP = form.selectbox("High Blood Pressure", ("Yes", "No"), index=None)
HighChol = form.selectbox("High Cholesterol", ("Yes", "No"), index=None)
CholCheck = form.selectbox("Have you check Cholesterol in the last 5 years", ("Yes", "No"), index=None, placeholder='Choose an option')
BMI = form.number_input("Enter your BMI",min_value=10, max_value=60, value='min')
Smoker = form.selectbox("Have you smoked up to 100 packs of ciggarette in your entire life", ("Yes", "No"), index=None)
Stroke = form.selectbox("Ever told you had stroke?", ("Yes", "No"), index=None)
HeartDiseaseorAttack = form.selectbox("Coronary heart disease (CHD) or Myocardial infarction (MI)", ("Yes", "No"), index=None)
PhysActivity = form.selectbox("Physical activity in the last 30 days", ("Yes", "No"), index=None)
Fruits = form.selectbox("Consume Fruit 1 or more times per day", ("Yes", "No"), index=None)
Veggies = form.selectbox("Consume Vegetables 1 or more times per day", ("Yes", "No"), index=None)
HvyAlcoholConsump = form.selectbox("Heavy drinkers (adult men having more than 14 drinks per week and adult women having more than 7 drinks per week)", ["Yes", "No"], index=None)
AnyHealthcare = form.selectbox("Have any kind of health care coverage, including health insurance, prepaid plans such as HMO, etc", ["Yes", "No"], index=None)
NoDocbcCost = form.selectbox("Was there a time in the past 12 months when you needed to see a doctor but could not because of cost?", ["Yes", "No"], index=None)
GenHlth = form.selectbox("How would you describe your general health", ["Excellent", "Very good","Good","Fair","Poor"], index=None)
...
DiffWalk = form.selectbox("Do you have serious difficulty walking or climbing stairs?", ["Yes", "No"], index=None)
Sex = form.selectbox("Select your gender", ["Male", "Female"], index=None)
Age = form.selectbox("Select Age Category", ["18-24", "24-30","30-36","36-42","42-48","48-54","54-58",
                                            "58-60","60-64","64-70","70-76","76-80","80+"], index=None)
...
Income = form.selectbox("Income Scale",['less than $10,000','$10,000-$15,000','$15,000-$20,000',
                                      ...
                                      '$55,000-$75,000','$75,000 or more'], index=None)
...
cat_data = ['HighBP', 'HighChol', 'CholCheck','Smoker', 'Stroke',
       ...
       'Income']
#user_data[cat_data] = user_data[cat_data].astype(np.int)
...
if st.sidebar.button("Predict"):
    if prediction == 0:
        st.markdown("Prediction: <font color='green'>**Non-diabetic**</font>", unsafe_allow_html=True)
    elif prediction == 1:
        st.markdown("Prediction: <font color='blue'>**Prediabetic**</font>", unsafe_allow_html=True)
    elif prediction == 2:
        st.markdown("Prediction: <font color='red'>**Diabetic**</font>", unsafe_allow_html=True)
